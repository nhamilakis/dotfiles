...
SITES=(
    ...
    ["cmd_not_found"]="https://command-not-found.com/"
)
...
main()
{
    ...
    then 
        ...
        if [ "$SITE_TO_USE" ]; then
            ...
            site=$(echo "$(gen_sites_list)" | rofi -dmenu -p "Search " -no-custom -theme themes/onedark.rasi)
            query=$(echo "$(gen_queries_list $site)" | rofi -dmenu  -p "Search ($site) " -theme themes/onedark.rasi)
